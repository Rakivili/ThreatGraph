...
// Writer sends adjacency rows to a remote HTTP endpoint.
type Writer struct {
	url     string
...
// WriteRows posts a batch of adjacency rows.
func (w *Writer) WriteRows(rows []*models.AdjacencyRow) error {
	...
	if err != nil {
		return fmt.Errorf("failed to marshal adjacency rows: %w", err)
	}
