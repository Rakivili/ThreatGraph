...
offline: build
	...
	./$(BIN_DIR)/$(APP)$(EXE) analyze --source clickhouse --config $(OFFLINE_CONFIG) --output $(OFFLINE_IIP) --tactical-output $(OFFLINE_TPG) --incident-output $(OFFLINE_INCIDENTS) --incident-min-seq $(OFFLINE_INCIDENT_MIN_SEQ) --subgraph-output-dir $(OFFLINE_SUBGRAPH_DIR)
